          ...
          <Table responsive hover size="sm" striped bordered className="rounded mb-0">
            <thead>
              <tr>
                ...
                <th>When</th>
                <th>Type {
                  this.clarifyPopup('Hình thức tham dự của thí sinh. \n* -1 -> SPECTATE, chỉ theo dõi, kết quả không được tính\n'+
...
class AddParticipationModal extends React.Component {
  ...
  submitHandler(e) {
    ...
    if (!addPartType) {alert('Hãy chọn 1 tư cách tham dự.'); return false;}
    users.trim();
    ...
    contestAPI.addContestParticipations({key: this.props.ckey, data})
    .then((res) => {
          ...
          <Form id="contest-participation-add-form" onSubmit={(e)=>this.submitHanlder(e)}>
            ...
            <div className="flex-center">
              {VIRTUAL_TYPE.map((type) => (
                <div key={`part-${type}`} className="flex-center">
                  <Form.Check inline name="add-participation-type" type="radio"
                    id={`add-participation-type-${type}`} label={`${type}`}
                    checked={type === addPartType} onChange={(e) => this.setState({ addPartType: type })}
                  />
                </div>
              ))}
            </div>
            ...
            <div className="flex-center">
              <em style={{fontSize: "12px"}}>
                Dán những tên người dùng (space-separated) mà bạn muốn đăng ký vào contest.
                Nếu có người dùng được thêm đã đăng ký (với tư cách là LIVE hoặc SPECTATE),
                hệ thống sẽ chỉnh lại Participation Type của họ.
              </em>
            </div>
          </Form>
        </Modal.Body>
