            ...
            <div className="org-item mt-1 mb-1 border" key={`org-${org.slug}`}>
              <div className="text-left ml-1">
                <p className="m-0">
                  ...
                  <sub> (Total Members: {org.member_count}, Actual Members: {org.real_member_count}, Suborgs: {org.suborg_count}) </sub>
                </p>
                <div className="d-flex ml-2">
                  <div style={{maxHeight: "50px", minHeight: "50px", maxWidth: "50px", minWidth: "50px"}}
                        className="text-center border" >{
                    org.logo_url ? <img style={{maxHeight: "100%", maxWidth: "100%"}} src={org.logo_url}/> : <>No Img</>
                  }</div>
                  <p className="m-0 ml-2">
                      ...
                      <br></br>
                      <strong>Hiển thị lên web?</strong>: {org.is_unlisted ? "Không" : "Có"}
                      <br></br>
                      <Link to={`/admin/org/${org.slug}`}>{`>> Detail`}</Link>
                      <Link className="pl-3" to="#" onClick={()=>this.props.setModalData(org.slug)}
                          >{`+ Add Sub Org`}</Link>
                  </p>
                </div>
              </div>
              <OrgList orgs={org.sub_orgs} prefix={prefix+`${idx+1}.`} setModalData={this.props.setModalData}/>
            </div>
          ))
        }
      </div>
  ...
  render() {
    ...
    return (
      <div className="admin admin-org">
        ...
        <NewModal show={this.state.modalData.show} parent={this.state.modalData.parentOrg}
        ...
        <div className="admin-options wrapper-vanilla m-0 mb-1">
          <div className="border d-inline-flex p-1" >
            <Button size="sm" style={{minHeight: "30px"}}
              ...
              onClick={(e) => this.openNewModal(null)}
            >
...
class NewModal extends React.Component {
  ...
  onSubmitHandler(e) {
    ...
    const endpoint = parent ? orgAPI.createSubOrg({parentSlug: parent, data}) : orgAPI.createOrg(data);

    endpoint.then((res)=> {
      toast.success("OK Created.");
      this.setState({ redirectUrl: `/admin/org/${this.state.slug.toUpperCase()}` })
    })
    .catch((err) => {
    ...
    return (
      <Modal show={this.props.show} onHide={()=>{
        ...
        <Modal.Body>
          ...
          <Form.Control size="sm" type="text" placeholder="Mã định danh cho tổ chức" id="new-org-slug"
                value={this.state.slug} onChange={(e)=>this.setState({slug: e.target.value})} required />
          <label className="required mb-0 mt-2"> Short name: </label>
          <Form.Control size="sm" type="text" placeholder="Tên ngắn của tổ chức, dùng để hiển thị trên scoreboard." id="new-org-slug"
                value={this.state.short_name} onChange={(e)=>this.setState({short_name: e.target.value})} required />
          <label className="required mb-0 mt-2"> Name: </label>
          <Form.Control size="sm" type="text" placeholder="Tên dài của tổ chức." id="new-org-slug"
                value={this.state.name} onChange={(e)=>this.setState({name: e.target.value})} required />
        </Modal.Body>
        <Modal.Footer>
          <Button variant="secondary" className="btn-svg"
                onClick={()=>{
                  this.setState({errors: null})
                  this.props.setHide()}
                }>
